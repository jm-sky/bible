  ...
  <div id="modal" class="modal fade" style="background-color: rgba(0,0,0,0.2)">
    <div class="modal-dialog modal-lg">
      <div class="modal-content p-1">
        ...
        <div class="results">
          <div
            v-for="(item, index) in $root.$app.searchResults"
            :key="index"
  ...
  methods: {
    seek(item) {
      ...
      this.$nextTick(() => {
        ...
        $linkedVerse
          .css({ "background-color": this.$root.$app.config.search.highlightColor })
          .delay(this.$root.$app.config.search.highlightTime)
          .queue(function(next) {
